...
def create_software(
    ...
    return stix2.Software(
        # id=Software.generate_id(name),
        id=stix2.Software(name=name,
                          version=version).id, # TODO: This is a hack to get around the fact that the id is not generated by the pycti library
        created_by_ref=created_by,
        name=name,
        confidence=confidence,
        object_marking_refs=object_markings,
...
def create_region(name: str, created_by: stix2.Identity) -> stix2.Location:
    ...
    return stix2.Location(
        ...
        created_by_ref=created_by,
        name=name,
        region=name,#NOTE:not sure if this is correct
